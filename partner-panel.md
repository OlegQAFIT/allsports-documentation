...
1. Абонент создаёт визит — по геолокации или сканированием QR-кода AllSports на объекте.

1. Визит появляется в партнёрской панели со статусом «ожидание».

1. Администратор нажимает «Проверить новые визиты» и подтверждает визит кнопкой «Принять».

![Иллюстрация](_assets/images/partner-panel/image4.png)

1. В приложении абонента появляется QR-код и таймер — подтверждение доступа к услуге.
...
1. Общая стоимость — суммарная стоимость всех визитов в белорусских рублях. Отображение информации зависит от роли пользователя в системе.
...
1. Неделя — визиты за выбранную неделю;

1. День — визиты за конкретный день;

1. Интервал — произвольный диапазон дат.
...
1. Принятые — визиты, успешно подтверждённые администратором;

1. Отклонённые — визиты, отклонённые администратором;

1. Тайм-аут — визиты, не подтверждённые в отведённое время.
...
1. В открывшемся окне при необходимости измените услугу или статус.

1. Укажите причину корректировки в поле «Причина корректировки».

1. Нажмите «Исправить» для отправки запроса на рассмотрение менеджером AllSports.
...
1. № — уникальный идентификатор визита для быстрого поиска при обращении в техническую поддержку;

1. Данные визита — детали визита до корректировки;

1. Запрос на корректировку — суть и причина поданного запроса;

1. Решение — статус обработки запроса. При положительном решении данные исходного визита будут изменены.
...
1. Описание — краткое описание объекта и предоставляемых услуг;

1. Правила посещения — условия, которые должны соблюдать абоненты;

1. Адрес — физический адрес объекта;

1. Контактный телефон — номера телефонов для связи (можно указать несколько через точку с запятой);

1. Веб-сайт — ссылка на сайт вашего объекта;

1. Режим работы — часы работы по каждому дню недели.
...
1. нажмите «За всё время» для отображения всех документов без ограничения по дате.
...
1. таблицу оказанных услуг с указанием количества визитов и стоимости по каждой услуге;
...
1. Электронная почта: support@allsports.by

1. Режим работы: понедельник–пятница 09:00–21:00, суббота–воскресенье 09:00–19:00

1. Мессенджеры: Telegram, Viber
