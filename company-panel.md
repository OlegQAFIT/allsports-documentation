...
1. Copay/B2C: Сотрудник самостоятельно по ссылке

#### Роль HR-менеджера:

1. B2B: Формирует список, задаёт уровни подписки, обновляет ежемесячно

1. Copay/B2C: Предоставляет ссылку сотрудникам, подтверждает или отклоняет заявки

#### Оплата подписки:

1. B2B: Компания платит полностью за всех сотрудников в списке

1. Copay: Оплата делится: компания платит часть, сотрудник — остаток

1. B2C: Компания оплачивает полностью или частично, но через механизм самостоятельной регистрации

#### Разделы меню:

1. B2B: Сотрудники, Счета, Акты, Аудит

1. Copay/B2C: Ссылка для регистрации, Активные подписки, Запросы для подтверждения, Отменённые, Счета, Акты, Аудит

#### Статистика (Engagement Report):

1. B2B: Доступна (при активации менеджером Allsports)

1. Copay/B2C: Не предусмотрена
...
1. Счета — счета на оплату

1. Акты — закрывающие акты

1. Аудит — журнал изменений

#### Разделы Copay/B2C-компании:

1. Ссылка для регистрации пользователей — персональная ссылка, которую вы направляете сотрудникам

1. Активные подписки — сотрудники с действующей подпиской

1. Запросы для подтверждения — новые заявки от сотрудников, ожидающие вашего одобрения

1. Отменённые — сотрудники, отменившие подписку
...
1. Скачать предыдущий — скачать список сотрудников за предыдущий период в Excel

1. Скачать данные предыдущего периода либо шаблон — шаблон для импорта

1. Импортировать — загрузить список сотрудников из Excel-файла

1. Импортировать сотрудников — альтернативная кнопка импорта

1. + Добавить / Добавить сотрудника — добавить одного сотрудника вручную
...
1. Номер телефона (логин в приложении)

1. Уровень подписки (Region, Lite, Classic, Premium, VIP)
...
1. <span class="doc-inline-icon"><img src="_assets/images/company-panel/image10.png" alt="Скачать предыдущий месяц"></span> скачать список сотрудников за предыдущий месяц

1. <span class="doc-inline-icon"><img src="_assets/images/company-panel/image11.png" alt="Импорт сотрудников"></span> импортировать список сотрудников (по предложенному шаблону)

1. <span class="doc-inline-icon"><img src="_assets/images/company-panel/image12.png" alt="Добавить сотрудника"></span> добавить сотрудника.

1. Кнопка «Сформировать счёт» — ручная генерация счёта на оплату
...
1. Стоимость – суммарная стоимость всех активных подписок сотрудников компании.
...
1. Введите номер мобильного телефона сотрудника в международном формате. Это логин сотрудника в приложении.

1. Выберите уровень подписки: Region, Lite, Classic, Premium или VIP.
...
1. Новая модель (Product Mix): отдельные столбцы для каждого уровня — Region, Lite, Classic, Premium, VIP.

#### Шаг 2. Заполните шаблон

1. Один сотрудник — одна строка.

1. Телефон в международном формате: +375291234567.

1. Не изменяйте заголовки столбцов и структуру файла.

1. Уровень подписки указывается точно как в шаблоне.
...
1. Система проверит данные и покажет результат валидации.

1. Поле «Фамилия Имя Отчество» должно содержать не менее двух слов — строка с одним словом отклоняется с сообщением «Введите полное ФИО сотрудника».

1. При наличии ошибок — исправьте строки и повторите загрузку.

1. При успешной проверке — подтвердите импорт.
...
1. если список сотрудников пуст, нажмите «Скопировать данные прошлого месяца?».

После запуска копирования система:

1. скопирует список сотрудников из предыдущего месяца;

1. проверит соответствие уровней подписки текущей модели;

1. отобразит строки с несоответствиями, если они будут обнаружены.

1. проверит поле «Фамилия Имя Отчество» на то же условие (не менее двух слов), что и при импорте через Excel.
...
1. Lite — 75+ объектов в Минске, 25+ видов активностей (тренажёрные залы, фитнес).

1. Classic — 600+ объектов по всей стране, 80+ вида активностей (танцы, стретчинг, единоборства).

1. Premium — 780+ объекта, 110+ видов активностей (теннис, картинг, СПА). Дополнительно 4 визита в объекты «Premium Plus».

1. VIP — 790+ объекта, 110+ видов активностей (массаж, конный спорт, сквош). Дополнительно 4 визита в объекты «VIP Plus».
...
1. ID заказа – идентификатор заказа, на основании которого был сформирован счет.

1. Количество – количество сотрудников или услуг, включенных в счет.

1. Всего – итоговая сумма счета.
...
1. Предпросмотр – открытие счета в области просмотра на странице.
...
1. Кол-во подписок — количество подписок, затронутых данным действием.

1. Цена — суммарная стоимость изменённых подписок в BYN.

1. ФИО — имя HR-менеджера или администратора системы, который выполнил действие. Можно сортировать по алфавиту.
...
1. Разграничение ответственности — если в компании несколько HR-менеджеров, аудит показывает, кто из них выполнил конкретное действие.

1. Выверка финансовых данных — при расхождениях в счётах аудит помогает установить, когда и какие изменения повлияли на итоговую сумму.

1. Подтверждение для бухгалтерии — журнал изменений служит дополнительным источником информации при сверке документов.
...
1. Стоимость подписки — полная стоимость.

1. Работодатель платит — доля компании.

1. Вы платите — доля сотрудника (0.00 BYN, если компания платит полностью).

Сотрудник заполняет форму:

1. Вводит ФИО.

1. Указывает адрес корпоративной почты.

1. Вводит номер мобильного телефона.

1. Выбирает уровень подписки (если доступно несколько).

1. Принимает условия обработки персональных данных и договор оферты.

1. Нажимает «Продолжить».
...
1. — заявка отклонена, сотрудник получает уведомление об отказе.
...
1. Unsubscribed — сотрудник самостоятельно отменил подписку
...
1. Active Memberships — число уникальных сотрудников, совершивших хотя бы 1 визит за прошлый месяц

1. % of Total Registered — доля активных от общего числа подписчиков
...
1. % от общего числа визитов (% of Total Visits)

1. Горизонтальный индикатор для наглядного сравнения
...
1. Суббота, воскресенье: выходной
